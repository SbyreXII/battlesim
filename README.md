...
- **PA/PM du joueur** : calculés comme `base stricte (6 PA / 3 PM) + bonus d'objets + bonus de Forgemagie (stuffFm.fm)`. Ce sont les 3 seules sources de PA/PM que dofusbook connaît lui-même (il n'a aucune information sur les quêtes de récompense du compte) — formule vérifiée exacte sur deux stuffs réels différents. Utiliser le champ "PA/tour" pour corriger si une quête spécifique change la donne.
- **PV du joueur** : `50 + 5×niveau + Vitalité` (formule donnée et vérifiée par un utilisateur, uniforme entre classes) — reste une approximation si les PV réels ne sont pas renseignés, car d'éventuels bonus de PV non modélisés (certains sorts/passifs de classe) ne sont pas comptés. Utiliser le champ "PV réels" pour corriger si besoin.
