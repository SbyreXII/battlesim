...
import { computeCharacterStats, basePlayerLifePoints } from "../src/lib/characterStats.js";
import { parseStuffJson } from "../src/lib/dofusbookParser.js";
import type { DofusbookStuff } from "../src/lib/dofusbookParser.js";

test("basePlayerLifePoints : 50 + 5×niveau (donné et vérifié par l'utilisateur, 1050 au niveau 200)", () => {
  assert.equal(basePlayerLifePoints(200), 1050);
  assert.equal(basePlayerLifePoints(1), 55);
  assert.equal(basePlayerLifePoints(0), 50);
});
...
test("PA/PM d'un vrai stuff Enutrof correspondent à l'affichage dofusbook.net (12 PA / 6 PM)", async () => {
  ...
  const stuff: DofusbookStuff = {
    ...
    // Bonus de Forgemagie reconstruit : l'extraction condensée d'origine
    // (via Claude in Chrome, cf. README) ne conservait pas `stuffFm`,
    // découvert plus tard comme la vraie source du +1 PA/+1 PM manquant sur
    // ce stuff (confirmé sur un autre stuff réel où stuffFm.fm = {pa:1, pm:1}
...
test("le bonus de Forgemagie (stuffFm.fm) s'ajoute au PA/PM, quelle que soit sa valeur", () => {
  const stuff: DofusbookStuff = {
    id: 0,
    name: "test",
    character_class: 1,
  ...
  const stats = computeCharacterStats(stuff);
  assert.equal(stats.actionPoints, 8); // 6 (base) + 2 (Forgemagie)
  assert.equal(stats.movementPoints, 3); // 3 (base) + 0 (Forgemagie)
});

test("PA/PM sans Forgemagie configurée retombent sur la base stricte (6 PA / 3 PM)", () => {
  const stuff: DofusbookStuff = {
    id: 0,
    name: "test",
    character_class: 1,
